...
def getDepth():
    ...
    body_bytes = flask.request.data
    # decode as cv2 image
    img = cv2.imdecode(np.frombuffer(body_bytes, np.uint8), cv2.IMREAD_COLOR)
    if img.ndim == 2:
    ...
    response = flask.Response(depth_bytes_f32, mimetype='application/octet-stream')
    response.headers["Access-Control-Allow-Origin"] = "*"
    response.headers["Access-Control-Allow-Headers"] = "*"
    response.headers["Access-Control-Allow-Methods"] = "*"
    response.headers["Access-Control-Expose-Headers"] = "*"
